...
const HangoutMessage = ({data}) => {
  const theme = useTheme();
  const navigation = useNavigation();
  const dispatch = useDispatch();
  const beingCreateOffer = useSelector((state) => state.feed.beingCreateOffer);
      ...
      {data.type === 1 && (
        <View style={styles.actionsWrapper}>
          ...
          {/* {shouldShowHangout && (
            <Button
              title='Hangout'
              loading={beingCreateOffer.includes(data.id)}
              containerStyle={styles.btnContainer}
              style={styles.actionBtn2}
              onPress={() =>
                dispatch(
                  createOffer({hangoutId: data.id, userId: data.user?.id}),
                )
              }
            />
          )} */}
