...
const EditCategoriesScreen = ({navigation, route}) => {
  ...
  const [selected, setSelected] = useState(userInfo?.categories?.data || []);
  ...
  const handleSubmit = () => {
    if (newSkills.length > 0) {
      ...
      dispatch(
        updateCategories(
          {categories: selected.map((i) => i.id)},
          {
            success: () => {
  ...
  const lang = {
    title: isEdit ? 'edit categories' : i18n.t('editCategories.title'),
    inputLabel: i18n.t('editCategories.inputLabel'),
    addYourSkills: i18n.t('editCategories.addYourSkills'),
    submit: isEdit ? 'Update' : i18n.t('editCategories.submit'),
    subTitle: i18n.t('editCategories.subTitle'),
    skipHint: i18n.t('editCategories.skipHint'),
    skip: i18n.t('editCategories.skip'),
    suggestedSkills: i18n.t('editCategories.suggestedSkills'),
  };
