...
const PickSpecialtyScreen = ({navigation, route}) => {
    ...
    if (newSkills.length > 0) {
      dispatch(
        addUserSpecialties(
          ...
          {
            success: (result) => {
              const userSpes = result?.data?.specialities?.data;
              const specs = [
                ...selected,
      ...
      const skillsToUpdate = [
        ...new Set(
          [...selected, ...userInfo?.specialities?.data].map((i) => i.id),
        ),
      ];
  ...
  // console.log('skillValues', skillValue);
