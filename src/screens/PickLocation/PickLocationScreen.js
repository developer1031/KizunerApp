...
const PickLocationScreen = ({navigation, route}) => {
  ...
  const handleFetchAddress = async (data) => {
    ...
    const route = address_components?.find((item) =>
    ...
    setSearch(handleStringAddress(political?.long_name, country?.long_name));
    setSearchShort(
      handleStringAddress(political?.long_name, country?.long_name),
    );

    //short && short?.short_name && setSearchShort(short?.short_name);

    setSearching(false);
  };

  const handleStringAddress = (item_one, item_two) => {
  ...
  const handlePressItem = async (data) => {
    ...
    if (detail) {
      ...
      // setSearchShort(data?.structured_formatting?.main_text);
