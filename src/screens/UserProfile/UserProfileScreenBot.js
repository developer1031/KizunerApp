...
import {FeedItem, HangoutPlaceholder} from 'components/FeedItem';
...
const UserProfileScreenBot = ({navigation, route}) => {
  ...
  const showOptions = () => {
    dispatch(
      showAlert({
        title: '',
        type: 'info',
        body: `You can't settings with ${data?.name}!`,
      }),
    );
    return;
  ...
  function renderItem({item}) {
    return <FeedItem data={item?.relation?.data} type={item?.type} />;
  }
            ...
            onPress={() => {
              dispatch(
                showAlert({
                  title: '',
                  type: 'info',
                  body: `You can't add friend ${data?.name}!`,
                }),
              );
              return;
            }}
                    ...
                    {data?.location?.address && (
                      <View style={stylesMain.locationWrap}>
                        <SimpleLineIcons
                          ...
                          size={getSize.f(17)}
                        />
                        <Text style={stylesMain.locationTxt}>
                          {data?.location?.address?.trimLeft()}
                        </Text>
                      </View>
                    )}
                  </View>
...
export default UserProfileScreenBot;
