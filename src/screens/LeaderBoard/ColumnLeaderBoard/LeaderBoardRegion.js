...
import {getListLeaderBoardRegional} from 'actions';
import FastImage from 'react-native-fast-image';
import _ from 'lodash';

const AnimatedLG = Animated.createAnimatedComponent(LinearGradient);

const LeaderBoardRegion = ({navigation, route}) => {
  ...
  const {top_10, top_3} = useSelector(
    (state) => state.trophy.leaderboardRegionalList,
  );
...
export default LeaderBoardRegion;
